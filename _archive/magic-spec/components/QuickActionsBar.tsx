...
export function QuickActionsBar() {
  ...
  const handleActionClick = (label: string) => {
    switch (label) {
      case 'Send Campaign': navigate('/magic/createemailcampaignpage'); break;
      case 'Schedule Meeting': navigate('/magic/activitiespage'); break;
      case 'Create Invoice': navigate('/magic/invoicedetailpage'); break;
      case 'Add Customer': navigate('/magic/addeditcustomerform'); break;
      case 'Create Task': navigate('/magic/activitiespage'); break;
      case 'Start Chat': navigate('/magic/conversationpage'); break;
    }
  };

  return <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
    {actions.map((action, index) => <motion.button key={action.label} onClick={() => handleActionClick(action.label)} whileHover={{
      scale: 1.02,
      y: -2
    }} whileTap={{
      scale: 0.98

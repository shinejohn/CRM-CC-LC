...
export function MarketingCampaignWizard() {
  ...
  return <div className="min-h-screen bg-[color:var(--nexus-fafaf8)]">
      {/* Header */}
      <div className="bg-white border-b border-[color:var(--nexus-e5e5e5)]">
        <div className="max-w-7xl mx-auto px-6 py-4">
          <div className="flex items-center gap-2 text-sm text-[color:var(--nexus-666666)] mb-2">
            <span>Learning Center</span>
            <ChevronRight className="w-4 h-4" />
            <span className="text-[color:var(--nexus-1e3a5f)] font-medium">
              Build Your Campaign
            </span>
          </div>
          <h1 className="text-2xl font-bold text-[color:var(--nexus-1a1a1a)]">
      ...
      {/* Progress Bar */}
      <div className="sticky top-0 z-40 bg-white border-b border-[color:var(--nexus-e5e5e5)] shadow-sm">
        <div className="max-w-7xl mx-auto px-6 py-4">
          <div className="flex items-center justify-between">
            {['Your Customer', 'What Works', 'Your Goal', 'Package Match', 'Plan It', 'Launch'].map((stepName, index) => <div key={stepName} className="flex items-center flex-1">
                <div className="flex flex-col items-center">
                  <div className={`w-10 h-10 rounded-full flex items-center justify-center font-bold transition-all ${index + 1 < state.currentStep ? 'bg-[color:var(--nexus-1e3a5f)] text-white' : index + 1 === state.currentStep ? 'bg-[color:var(--nexus-1e3a5f)] text-white ring-4 ring-[color:var(--nexus-1e3a5f)]/20' : 'bg-[color:var(--nexus-e5e5e5)] text-[color:var(--nexus-666666)]'}`}>
                    {index + 1 < state.currentStep ? <Check className="w-5 h-5" /> : index + 1}
                  </div>
                  <span className={`text-xs mt-2 font-medium ${index + 1 === state.currentStep ? 'text-[color:var(--nexus-1e3a5f)]' : 'text-[color:var(--nexus-666666)]'}`}>
                    {stepName}
                  </span>
                </div>
                {index < 5 && <div className={`flex-1 h-1 mx-4 rounded-full transition-all ${index + 1 < state.currentStep ? 'bg-[color:var(--nexus-1e3a5f)]' : 'bg-[color:var(--nexus-e5e5e5)]'}`} />}
              </div>)}
          </div>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-6 py-8">
        <div className="flex gap-8">
          ...
          <div className="flex-1">
            <AnimatePresence mode="wait">
              ...
              duration: 0.3
                ...
                {/* Title */}
                <div className="mb-8">
                  <h2 className="text-3xl font-bold text-[color:var(--nexus-1a1a1a)] mb-2">
                    {currentScreen.title}
                  </h2>
                  {currentScreen.subtitle && <p className="text-lg text-[color:var(--nexus-666666)]">
                ...
                {state.currentScreenIndex === 0 && communities.length > 0 && (
                  <div className="mb-8 p-6 bg-white rounded-xl border border-[color:var(--nexus-e5e5e5)]">
                    <h3 className="font-bold text-[color:var(--nexus-1a1a1a)] mb-2 flex items-center gap-2">
                      <MapPin className="w-5 h-5 text-[color:var(--nexus-1e3a5f)]" />
                      Target by community
                    </h3>
                    <p className="text-sm text-[color:var(--nexus-666666)] mb-4">
                    ...
                    <select
                      ...
                      className="w-full px-4 py-2 border border-[color:var(--nexus-e5e5e5)] rounded-lg text-[color:var(--nexus-1a1a1a)] focus:ring-2 focus:ring-[color:var(--nexus-1e3a5f)]/20 focus:border-[color:var(--nexus-1e3a5f)]"
                    >
                    ...
                    {state.communityId && businessCount != null && (
                      <p className="mt-2 text-sm text-[color:var(--nexus-666666)] flex items-center gap-1">
                ...
                {/* Navigation */}
                <div className="flex items-center justify-between mt-12 pt-8 border-t border-[color:var(--nexus-e5e5e5)]">
                  <button onClick={handleBack} disabled={!currentScreen.navigation.backEnabled} className="flex items-center gap-2 px-6 py-3 text-[color:var(--nexus-666666)] hover:text-[color:var(--nexus-1a1a1a)] font-medium transition-colors disabled:opacity-50 disabled:cursor-not-allowed">
                  ...
                  {launchSuccess ? (
                    ...
                    <>
                      ...
                      <button
                        onClick={() => handleNext()}
                        disabled={launching}
                        className="flex items-center gap-2 px-8 py-3 bg-[color:var(--nexus-1e3a5f)] text-white rounded-lg font-bold hover:bg-[color:var(--nexus-2d5a8a)] transition-colors shadow-lg disabled:opacity-70 disabled:cursor-not-allowed"
                      >
          ...
          opacity: 1,
          x: 0
        }} className="w-80 bg-white rounded-xl border border-[color:var(--nexus-e5e5e5)] p-6 h-fit sticky top-24">
              <h3 className="font-bold text-[color:var(--nexus-1a1a1a)] mb-4">Your Progress</h3>
              <div className="space-y-4">
                {state.businessName && <div>
                    <div className="text-sm text-[color:var(--nexus-666666)] mb-1">Business</div>
                    <div className="font-medium text-[color:var(--nexus-1a1a1a)]">
                      {state.businessName}
                    </div>
                  </div>}
                {state.community && <div>
                    <div className="text-sm text-[color:var(--nexus-666666)] mb-1">Community</div>
                    <div className="font-medium text-[color:var(--nexus-1a1a1a)]">
                      {state.community}
                    </div>
                  </div>}
                {state.businessCategory && <div>
                    <div className="text-sm text-[color:var(--nexus-666666)] mb-1">
                      Business Type
                    </div>
                    <div className="font-medium text-[color:var(--nexus-1a1a1a)] capitalize">
                ...
                <div>
                  ...
                  <div className="font-medium text-[color:var(--nexus-8b5cf6)] capitalize">
                ...
                <div>
                  ...
                  <div className="font-bold text-xl text-[color:var(--nexus-1a1a1a)]">
...
function QuestionBlockComponent({
  ...
  if (block.inputType === 'text' || block.inputType === 'textarea') {
    return <div className="bg-white rounded-xl border border-[color:var(--nexus-e5e5e5)] p-8">
        <h3 className="text-xl font-bold text-[color:var(--nexus-1a1a1a)] mb-2">
          {block.question}
        </h3>
        {block.helpText && <p className="text-[color:var(--nexus-666666)] mb-6">{block.helpText}</p>}
        {block.inputType === 'textarea' ? <textarea value={typeof selected === 'string' ? selected : ''} onChange={e => handleTextChange(e.target.value)} placeholder={block.placeholder} className="w-full px-4 py-3 border-2 border-[color:var(--nexus-e5e5e5)] rounded-lg focus:outline-none focus:border-[color:var(--nexus-1e3a5f)] transition-colors resize-none" rows={4} /> : <input type="text" value={typeof selected === 'string' ? selected : ''} onChange={e => handleTextChange(e.target.value)} placeholder={block.placeholder} className="w-full px-4 py-3 border-2 border-[color:var(--nexus-e5e5e5)] rounded-lg focus:outline-none focus:border-[color:var(--nexus-1e3a5f)] transition-colors" />}
      </div>;
  }
  return <div className="bg-white rounded-xl border border-[color:var(--nexus-e5e5e5)] p-8">
      <h3 className="text-xl font-bold text-[color:var(--nexus-1a1a1a)] mb-2">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        return <button key={option.id} onClick={() => handleSelect(option.id)} className={`p-6 rounded-lg border-2 transition-all text-left ${isSelected ? 'border-[color:var(--nexus-1e3a5f)] bg-[color:var(--nexus-1e3a5f)]/5' : 'border-[color:var(--nexus-e5e5e5)] hover:border-[color:var(--nexus-1e3a5f)]/30'}`}>
              <div className="flex items-start gap-4">
                {option.icon && <span className="text-3xl">{option.icon}</span>}
                <div className="flex-1">
                  <div className="font-bold text-[color:var(--nexus-1a1a1a)] mb-1">
                    {option.label}
                  </div>
                  {option.description && <div className="text-sm text-[color:var(--nexus-666666)] mb-2">
                      {option.description}
                    </div>}
                  {option.example && <div className="text-xs text-[color:var(--nexus-666666)] italic">
                      Example: {option.example}
                    </div>}
                </div>
                <div className={`w-6 h-6 rounded-full border-2 flex items-center justify-center flex-shrink-0 ${isSelected ? 'bg-[color:var(--nexus-1e3a5f)] border-[color:var(--nexus-1e3a5f)]' : 'border-[color:var(--nexus-e5e5e5)]'}`}>
...
function RankingBlockComponent({
  ...
  return <div className="bg-white rounded-xl border border-[color:var(--nexus-e5e5e5)] p-8">
      <h3 className="text-xl font-bold text-[color:var(--nexus-1a1a1a)] mb-6">
        {block.instruction}
      </h3>
      <p className="text-[color:var(--nexus-666666)] mb-6">Click to rank (1st, 2nd, 3rd)</p>

      <div className="space-y-4">
        {block.items.map(item => {
        const rank = rankedItems.indexOf(item.id) + 1;
        return <button key={item.id} onClick={() => handleRank(item.id)} className={`w-full p-6 rounded-lg border-2 transition-all text-left ${rank > 0 ? 'border-[color:var(--nexus-1e3a5f)] bg-[color:var(--nexus-1e3a5f)]/5' : 'border-[color:var(--nexus-e5e5e5)] hover:border-[color:var(--nexus-1e3a5f)]/30'}`}>
              <div className="flex items-start gap-4">
                {rank > 0 && <div className="w-10 h-10 rounded-full bg-[color:var(--nexus-1e3a5f)] text-white flex items-center justify-center font-bold flex-shrink-0">
                    {rank}
                  </div>}
                ...
                <div className="flex-1">
                  ...
                  {item.description && <div className="text-sm text-[color:var(--nexus-666666)] mb-2">
                      {item.description}
                    </div>}
                  {item.example && <div className="text-xs text-[color:var(--nexus-666666)] italic">
...
function InfoBlockComponent({
  ...
  const variantStyles = {
    tip: 'border-l-4 border-[color:var(--nexus-00cc6a)] bg-[color:var(--nexus-e6fff2)]',
    insight: 'border-l-4 border-[color:var(--nexus-1e3a5f)] bg-[color:var(--nexus-1e3a5f)]/5',
    example: 'border-l-4 border-[color:var(--nexus-8b5cf6)] bg-[color:var(--nexus-8b5cf6)]/5',
    warning: 'border-l-4 border-[color:var(--nexus-ffb800)] bg-[color:var(--nexus-fff9e6)]'
  };
  const icons = {
    tip: <Lightbulb className="w-5 h-5 text-[color:var(--nexus-00cc6a)]" />,
    insight: <Sparkles className="w-5 h-5 text-[color:var(--nexus-1e3a5f)]" />,
    example: <Sparkles className="w-5 h-5 text-[color:var(--nexus-8b5cf6)]" />,
    warning: <AlertCircle className="w-5 h-5 text-[color:var(--nexus-ffb800)]" />
  };
  return <div className={`p-6 rounded-lg ${variantStyles[block.variant]}`}>
      <div className="flex items-start gap-3">
        {icons[block.variant]}
        <div className="flex-1">
          <h4 className="font-bold text-[color:var(--nexus-1a1a1a)] mb-2">{block.title}</h4>
          <p className="text-[color:var(--nexus-1a1a1a)]/80 leading-relaxed whitespace-pre-line">
            {block.content}
          </p>
        </div>
      </div>
...
function SelectionGridBlockComponent({
  ...
  return <div className="bg-white rounded-xl border border-[color:var(--nexus-e5e5e5)] p-8">
      {block.instruction && <p className="text-[color:var(--nexus-666666)] mb-6">{block.instruction}</p>}

      <div className={`grid ${gridCols[block.columns]} gap-4`}>
        {block.items.map(item => <button key={item.id} onClick={() => handleSelect(item.id)} className={`p-6 rounded-lg border-2 transition-all ${isSelected(item.id) ? 'border-[color:var(--nexus-1e3a5f)] bg-[color:var(--nexus-1e3a5f)]/5' : 'border-[color:var(--nexus-e5e5e5)] hover:border-[color:var(--nexus-1e3a5f)]/30'}`}>
            <div className="text-center">
              <div className="text-4xl mb-3">{item.icon}</div>
              <div className="font-bold text-[color:var(--nexus-1a1a1a)] mb-1">{item.label}</div>
              {item.description && <div className="text-sm text-[color:var(--nexus-666666)]">{item.description}</div>}
              {item.tag && <div className="mt-2 inline-block px-2 py-1 bg-[color:var(--nexus-ffb800)]/20 text-[color:var(--nexus-ffb800)] text-xs font-medium rounded">
                  {item.tag}
                </div>}
            </div>
          </button>)}
...
function PackageComparisonBlockComponent({
  ...
  return <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
      {block.packages.map(pkg => <div key={pkg.id} className={`bg-white rounded-xl border-2 p-6 ${pkg.isRecommended ? 'border-[color:var(--nexus-1e3a5f)] shadow-xl' : 'border-[color:var(--nexus-e5e5e5)]'}`}>
          {pkg.isRecommended && <div className="mb-4 px-3 py-1 bg-[color:var(--nexus-1e3a5f)] text-white text-sm font-bold rounded inline-block">
              ⭐ RECOMMENDED
            </div>}

          <h3 className="text-xl font-bold text-[color:var(--nexus-1a1a1a)] mb-2">{pkg.name}</h3>
          <p className="text-[color:var(--nexus-666666)] mb-4">{pkg.tagline}</p>

          <div className="mb-6">
            <div className="text-3xl font-bold text-[color:var(--nexus-1a1a1a)] mb-1">
              {pkg.price}
            </div>
            {pkg.priceNote && <div className="text-sm text-[color:var(--nexus-666666)]">{pkg.priceNote}</div>}
          </div>

          <div className="space-y-3 mb-6">
            {pkg.highlights.map((highlight, idx) => <div key={idx} className="flex items-start gap-2">
                <Check className="w-5 h-5 text-[color:var(--nexus-00cc6a)] flex-shrink-0 mt-0.5" />
                <span className="text-[color:var(--nexus-1a1a1a)]">{highlight}</span>
              </div>)}
          </div>

          <div className="pt-4 border-t border-[color:var(--nexus-e5e5e5)]">
            <div className="text-sm text-[color:var(--nexus-666666)] mb-2">Best for:</div>
            <div className="text-[color:var(--nexus-1a1a1a)] font-medium">{pkg.bestFor}</div>
          </div>

          <button onClick={() => onSelect(pkg.id)} className={`w-full mt-6 py-3 rounded-lg font-bold transition-all ${pkg.isRecommended ? 'bg-[color:var(--nexus-1e3a5f)] text-white hover:bg-[color:var(--nexus-2d5a8a)]' : 'bg-[color:var(--nexus-e5e5e5)] text-[color:var(--nexus-1a1a1a)] hover:bg-[color:var(--nexus-e5e5e5)]/70'}`}>
            Select This
          </button>
        </div>)}
    </div>;
...
function SummaryBlockComponent({
  ...
  return <div className="bg-white rounded-xl border border-[color:var(--nexus-e5e5e5)] p-8">
      <h3 className="text-2xl font-bold text-[color:var(--nexus-1a1a1a)] mb-6">{block.title}</h3>

      <div className="space-y-6">
        {block.sections.map((section, idx) => <div key={idx}>
            <h4 className="font-bold text-[color:var(--nexus-1a1a1a)] mb-3">{section.heading}</h4>
            <div className="space-y-2">
              {section.items.map((item, itemIdx) => <div key={itemIdx} className="flex items-center justify-between p-3 bg-[color:var(--nexus-fafaf8)] rounded-lg">
                  <div className="flex items-center gap-2">
                    {item.included && <Check className="w-5 h-5 text-[color:var(--nexus-00cc6a)]" />}
                    <span className="text-[color:var(--nexus-1a1a1a)]">{item.label}</span>
                    {item.note && <span className="text-xs text-[color:var(--nexus-666666)]">
                        ({item.note})
                      </span>}
                  </div>
                  {item.value && <span className="font-medium text-[color:var(--nexus-1a1a1a)]">
                      {item.value}
                    </span>}
                </div>)}
            </div>
      ...
      {block.total && <div className="mt-6 pt-6 border-t-2 border-[color:var(--nexus-e5e5e5)]">
          <div className="flex items-center justify-between mb-2">
            <span className="text-lg font-bold text-[color:var(--nexus-1a1a1a)]">
              {block.total.label}
            </span>
            <span className="text-2xl font-bold text-[color:var(--nexus-1a1a1a)]">
              {block.total.value}
            </span>
          </div>
          {block.total.note && <p className="text-sm text-[color:var(--nexus-666666)]">{block.total.note}</p>}
        </div>}
    </div>;
}
function InputBlockComponent({
  ...
  return <div className="bg-white rounded-xl border border-[color:var(--nexus-e5e5e5)] p-8">
      <label className="block mb-2 font-bold text-[color:var(--nexus-1a1a1a)]">
        {block.label}
      </label>
      {block.helpText && <p className="text-sm text-[color:var(--nexus-666666)] mb-4">{block.helpText}</p>}

      {block.inputType === 'textarea' ? <textarea value={inputValue} placeholder={block.placeholder} onChange={e => handleChange(e.target.value)} className="w-full px-4 py-3 border-2 border-[color:var(--nexus-e5e5e5)] rounded-lg focus:outline-none focus:border-[color:var(--nexus-1e3a5f)] transition-colors resize-none" rows={4} /> : <input type={block.inputType} value={inputValue} placeholder={block.placeholder} onChange={e => handleChange(e.target.value)} className="w-full px-4 py-3 border-2 border-[color:var(--nexus-e5e5e5)] rounded-lg focus:outline-none focus:border-[color:var(--nexus-1e3a5f)] transition-colors" />}
    </div>;
}
function ChecklistBlockComponent({
  ...
  return <div className="bg-white rounded-xl border border-[color:var(--nexus-e5e5e5)] p-8">
      <p className="text-[color:var(--nexus-666666)] mb-6">{block.instruction}</p>

      <div className="space-y-4">
        {block.items.map(item => <label key={item.id} className="flex items-start gap-4 p-4 rounded-lg border-2 border-[color:var(--nexus-e5e5e5)] hover:border-[color:var(--nexus-1e3a5f)]/30 cursor-pointer transition-all">
            <input type="checkbox" checked={selected.includes(item.id)} onChange={() => handleToggle(item.id)} className="w-5 h-5 text-[color:var(--nexus-1e3a5f)] border-[color:var(--nexus-e5e5e5)] rounded focus:ring-2 focus:ring-[color:var(--nexus-1e3a5f)]/20 mt-0.5" />
            <div className="flex-1">
              <div className="font-bold text-[color:var(--nexus-1a1a1a)] mb-1">{item.label}</div>
              {item.description && <div className="text-sm text-[color:var(--nexus-666666)]">{item.description}</div>}
            </div>
            {item.price && <div className="font-bold text-[color:var(--nexus-1a1a1a)]">{item.price}</div>}
          </label>)}
      </div>
    </div>;
}
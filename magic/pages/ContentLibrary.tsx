      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
        ...
        <div className="bg-white rounded-xl border border-slate-200 p-6 mb-6">
          <div className="flex flex-wrap items-center gap-3 mb-4">
            {filters.map(filter => <button key={filter.id} onClick={() => setActiveFilter(filter.id as any)} className={`px-4 py-2 rounded-lg font-medium text-sm transition-colors ${activeFilter === filter.id ? 'bg-[#1E3A5F] text-white' : 'bg-slate-100 text-slate-700 hover:bg-slate-200'}`}>
                {filter.label}
              </button>)}
          </div>
          ...
          <div className="flex gap-4">
            <div className="flex-1 relative">
              ...
              <input type="text" value={searchQuery} onChange={e => setSearchQuery(e.target.value)} placeholder="Search content..." className="w-full pl-10 pr-4 py-3 border-2 border-slate-200 rounded-lg focus:border-[#1E3A5F] focus:outline-none" />
            </div>

            <div className="relative">
              <select value={sortBy} onChange={e => setSortBy(e.target.value)} className="appearance-none px-4 py-3 pr-10 border-2 border-slate-200 rounded-lg focus:border-[#1E3A5F] focus:outline-none bg-white font-medium text-slate-700">
        ...
        <div className="space-y-8">
          ...
          {(activeFilter === 'all' || activeFilter === 'ad') && <div>
              ...
              opacity: 1,
              y: 0
            }} className="bg-white rounded-xl border-2 border-slate-200 overflow-hidden hover:border-[#1E3A5F] transition-all group">
                      {/* Thumbnail */}
                      ...
                      <div className="p-4">
                        ...
                        <div className="flex gap-2">
                          ...
                          <button className="flex-1 px-3 py-2 bg-[#1E3A5F] hover:bg-[#152d4a] text-white rounded-lg text-sm font-medium transition-colors flex items-center justify-center gap-1">
          ...
          {getFilteredItems().length === 0 && <div className="bg-white rounded-xl border border-slate-200 p-12 text-center">
              ...
              <button onClick={() => onNavigate?.('content-type-selection')} className="px-6 py-3 bg-[#1E3A5F] hover:bg-[#152d4a] text-white rounded-lg font-medium transition-colors">
                Create Content
              </button>
            </div>}
        </div>

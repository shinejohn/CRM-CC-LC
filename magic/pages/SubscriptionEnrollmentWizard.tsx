...
export function SubscriptionEnrollmentWizard({
  ...
  return <div className="min-h-screen bg-gradient-to-br from-slate-50 to-blue-50 py-12 px-4">
      <div className="max-w-5xl mx-auto">
          ...
          <div className="flex items-center justify-between mb-4">
            ...
            length: totalSteps
                ...
                <div className="flex items-center">
                  <div className={`w-10 h-10 rounded-full flex items-center justify-center font-bold transition-all ${index + 1 < currentStep ? 'bg-[#1E3A5F] text-white' : index + 1 === currentStep ? 'bg-[#1E3A5F] text-white ring-4 ring-[#1E3A5F]/20' : 'bg-white text-slate-400 border-2 border-slate-200'}`}>
                    {index + 1 < currentStep ? <Check className="w-5 h-5" /> : index + 1}
                  </div>
                </div>
                {index < totalSteps - 1 && <div className={`flex-1 h-1 mx-2 rounded-full transition-all ${index + 1 < currentStep ? 'bg-[#1E3A5F]' : 'bg-slate-200'}`} />}
              </Fragment>)}
          </div>
        </div>
        ...
        <AnimatePresence mode="wait">
          ...
          x: -20
              ...
              return <motion.button key={type.id} variants={itemVariants} whileHover={{
                ...
                scale: 0.98
              }} onClick={() => handleBusinessTypeSelect(type.id)} className={`relative p-6 rounded-xl border-2 transition-all text-left ${isSelected ? 'border-[#1E3A5F] bg-[#1E3A5F]/5 shadow-lg' : 'border-slate-200 hover:border-slate-300 bg-white'}`}>
                      {isSelected && <motion.div initial={{
                  scale: 0
                }} animate={{
                  scale: 1
                }} className="absolute top-4 right-4 w-6 h-6 bg-[#1E3A5F] rounded-full flex items-center justify-center">
                          <Check className="w-4 h-4 text-white" />
                        </motion.div>}
                      <div className={`w-12 h-12 rounded-lg flex items-center justify-center mb-4 ${isSelected ? 'bg-[#1E3A5F] text-white' : 'bg-slate-100 text-slate-600'}`}>
                        <Icon className="w-6 h-6" />
                      </div>
                      <h3 className={`font-bold mb-1 ${isSelected ? 'text-[#1E3A5F]' : 'text-slate-900'}`}>
                        {type.label}
                      </h3>
          ...
          x: -20
              ...
              return <motion.button key={goal.id} variants={itemVariants} whileHover={{
                ...
                scale: 0.98
              }} onClick={() => handleGoalToggle(goal.id)} className={`relative p-6 rounded-xl border-2 transition-all text-left h-full ${isSelected ? 'border-[#1E3A5F] bg-[#1E3A5F]/5 shadow-lg' : 'border-slate-200 hover:border-slate-300 bg-white'}`}>
                      <div className="flex items-start justify-between mb-4">
                        <div className={`w-10 h-10 rounded-lg flex items-center justify-center ${isSelected ? 'bg-[#1E3A5F] text-white' : 'bg-slate-100 text-slate-600'}`}>
                          <Icon className="w-5 h-5" />
                        </div>
                        {isSelected && <div className="w-6 h-6 bg-[#1E3A5F] rounded-full flex items-center justify-center">
                            <Check className="w-4 h-4 text-white" />
                          </div>}
                      </div>
                      <h3 className={`font-bold mb-2 ${isSelected ? 'text-[#1E3A5F]' : 'text-slate-900'}`}>
                        {goal.label}
                      </h3>
              ...
              return <motion.button key={pub.id} variants={itemVariants} whileHover={{
                ...
                backgroundColor: `${pub.color}10`
                  ...
                  backgroundColor: pub.color
                    ...
                    backgroundColor: isSelected ? pub.color : '#F1F5F9',
          ...
          x: -20
              ...
              return <motion.div key={pkg.id} variants={itemVariants} className={`relative rounded-2xl border-2 transition-all overflow-hidden flex flex-col h-full ${isSelected ? 'border-[#1E3A5F] shadow-xl ring-4 ring-[#1E3A5F]/10 scale-105 z-10' : isSoldOut ? 'border-slate-100 bg-slate-50 opacity-80' : 'border-slate-200 hover:border-slate-300 bg-white hover:shadow-lg'}`}>
                      {isSelected && <div className="bg-[#1E3A5F] text-white text-center py-1.5 text-xs font-bold uppercase tracking-wider">
                          Selected Package
                        </div>}
                      ...
                      <div className="p-6 flex-1 flex flex-col">
                        ...
                        <div className="mt-auto">
                          ...
                          <button onClick={() => !isSoldOut && handlePackageSelect(pkg.id)} disabled={isSoldOut} className={`w-full py-3 rounded-xl font-bold transition-colors ${isSelected ? 'bg-[#1E3A5F] text-white' : isSoldOut ? 'bg-slate-200 text-slate-400 cursor-not-allowed' : 'bg-slate-100 text-slate-700 hover:bg-slate-200'}`}>
          ...
          x: -20
              ...
              scale: 0.98
            }} onClick={() => setBillingCycle('monthly')} className={`p-8 rounded-2xl border-2 transition-all text-center ${billingCycle === 'monthly' ? 'border-[#1E3A5F] bg-[#1E3A5F]/5 shadow-lg relative overflow-hidden' : 'border-slate-200 hover:border-slate-300 bg-white'}`}>
                  ...
                  {billingCycle === 'monthly' && <div className="absolute top-4 right-4 w-6 h-6 bg-[#1E3A5F] rounded-full flex items-center justify-center">
                      <Check className="w-4 h-4 text-white" />
                    </div>}
                </motion.button>
              ...
              scale: 0.98
            }} onClick={() => setBillingCycle('annual')} className={`p-8 rounded-2xl border-2 transition-all text-center relative overflow-hidden ${billingCycle === 'annual' ? 'border-[#1E3A5F] bg-[#1E3A5F]/5 shadow-lg' : 'border-slate-200 hover:border-slate-300 bg-white'}`}>
                  ...
                  {billingCycle === 'annual' && <div className="absolute top-4 left-4 w-6 h-6 bg-[#1E3A5F] rounded-full flex items-center justify-center">
                      <Check className="w-4 h-4 text-white" />
                    </div>}
                </motion.button>
              </div>
                ...
                <div className="space-y-4">
                  <div>
                    ...
                    <div className="relative">
                      ...
                      <input type="text" placeholder="0000 0000 0000 0000" className="w-full pl-10 pr-4 py-3 rounded-lg border border-slate-300 focus:ring-2 focus:ring-[#1E3A5F] focus:border-transparent outline-none" />
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <input type="text" placeholder="MM/YY" className="w-full px-4 py-3 rounded-lg border border-slate-300 focus:ring-2 focus:ring-[#1E3A5F] focus:border-transparent outline-none" />
                    </div>
                    <div>
                      <label className="block text-sm font-medium text-slate-700 mb-1">
                        CVC
                      </label>
                      <input type="text" placeholder="123" className="w-full px-4 py-3 rounded-lg border border-slate-300 focus:ring-2 focus:ring-[#1E3A5F] focus:border-transparent outline-none" />
                    </div>
                  </div>
                  ...
                  <div>
                    ...
                    <input type="text" placeholder="12345" className="w-full px-4 py-3 rounded-lg border border-slate-300 focus:ring-2 focus:ring-[#1E3A5F] focus:border-transparent outline-none" />
                  </div>
                </div>
              </div>
            </motion.div>}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 max-w-4xl mx-auto">
                <div className="space-y-4">
                  <div>
                    ...
                    <input type="text" value={businessInfo.name} onChange={e => handleBusinessInfoChange('name', e.target.value)} className="w-full px-4 py-2.5 rounded-lg border border-slate-300 focus:ring-2 focus:ring-[#1E3A5F] focus:border-transparent outline-none" />
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-slate-700 mb-1">
                      Category *
                    </label>
                    <input type="text" value={businessInfo.category} onChange={e => handleBusinessInfoChange('category', e.target.value)} className="w-full px-4 py-2.5 rounded-lg border border-slate-300 focus:ring-2 focus:ring-[#1E3A5F] focus:border-transparent outline-none" />
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-slate-700 mb-1">
                      Website
                    </label>
                    <input type="text" value={businessInfo.website} onChange={e => handleBusinessInfoChange('website', e.target.value)} className="w-full px-4 py-2.5 rounded-lg border border-slate-300 focus:ring-2 focus:ring-[#1E3A5F] focus:border-transparent outline-none" />
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-slate-700 mb-1">
                      Phone *
                    </label>
                    <input type="text" value={businessInfo.phone} onChange={e => handleBusinessInfoChange('phone', e.target.value)} className="w-full px-4 py-2.5 rounded-lg border border-slate-300 focus:ring-2 focus:ring-[#1E3A5F] focus:border-transparent outline-none" />
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-slate-700 mb-1">
                      Email *
                    </label>
                    <input type="email" value={businessInfo.email} onChange={e => handleBusinessInfoChange('email', e.target.value)} className="w-full px-4 py-2.5 rounded-lg border border-slate-300 focus:ring-2 focus:ring-[#1E3A5F] focus:border-transparent outline-none" />
                  </div>
                </div>
                ...
                <div className="space-y-4">
                  <div>
                    ...
                    <input type="text" value={businessInfo.address} onChange={e => handleBusinessInfoChange('address', e.target.value)} className="w-full px-4 py-2.5 rounded-lg border border-slate-300 focus:ring-2 focus:ring-[#1E3A5F] focus:border-transparent outline-none" />
                  </div>
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      <label className="block text-sm font-medium text-slate-700 mb-1">
                      ...
                      <input type="text" value={businessInfo.city} onChange={e => handleBusinessInfoChange('city', e.target.value)} className="w-full px-4 py-2.5 rounded-lg border border-slate-300 focus:ring-2 focus:ring-[#1E3A5F] focus:border-transparent outline-none" />
                    </div>
                    <div>
                      <label className="block text-sm font-medium text-slate-700 mb-1">
                        State *
                      </label>
                      <input type="text" value={businessInfo.state} onChange={e => handleBusinessInfoChange('state', e.target.value)} className="w-full px-4 py-2.5 rounded-lg border border-slate-300 focus:ring-2 focus:ring-[#1E3A5F] focus:border-transparent outline-none" />
                    </div>
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-slate-700 mb-1">
                    ...
                    <input type="text" value={businessInfo.zip} onChange={e => handleBusinessInfoChange('zip', e.target.value)} className="w-full px-4 py-2.5 rounded-lg border border-slate-300 focus:ring-2 focus:ring-[#1E3A5F] focus:border-transparent outline-none" />
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-slate-700 mb-1">
                      Description
                    </label>
                    <textarea rows={4} value={businessInfo.description} onChange={e => handleBusinessInfoChange('description', e.target.value)} className="w-full px-4 py-2.5 rounded-lg border border-slate-300 focus:ring-2 focus:ring-[#1E3A5F] focus:border-transparent outline-none resize-none" placeholder="Briefly describe your business..." />
                  </div>
                </div>
              </div>
            </motion.div>}
        ...
        }} onClick={handleNext} disabled={!canProceed()} className={`flex items-center gap-2 px-8 py-3 rounded-xl font-bold transition-all ${canProceed() ? 'bg-[#1E3A5F] text-white hover:bg-[#1E3A5F]/90 shadow-lg shadow-[#1E3A5F]/30' : 'bg-slate-200 text-slate-400 cursor-not-allowed'}`}>

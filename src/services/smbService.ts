...
export const smbService = {
  getProfile: () =>
    apiClient.get<ApiResponse<SMBProfile>>('/smb/profile').then((r: any) => r.data.data),

  updateProfile: (data: Partial<SMBProfile>) =>
    apiClient.put<ApiResponse<SMBProfile>>('/smb/profile', data).then((r: any) => r.data.data),

  /** Intelligence Hub: full aggregated profile */
  getFullProfile: (id: string) =>
    apiClient.get<ApiResponse<SMBFullProfile>>(`/smb/${id}/full-profile`).then((r: any) => r.data.data),

  /** Intelligence Hub: AI-relevant fields for prompt injection */
  getAIContext: (id: string) =>
    apiClient.get<ApiResponse<Record<string, unknown>>>(`/smb/${id}/ai-context`).then((r: any) => r.data.data),

  /** Intelligence Hub: condensed text summary for system prompt */
  getIntelligenceSummary: (id: string) =>
    apiClient.get<ApiResponse<{ summary: string }>>(`/smb/${id}/intelligence-summary`).then((r: any) => r.data.data.summary),
  ...
  updateSection: (id: string, section: SMBProfileSectionKey, data: Record<string, unknown>) =>
    apiClient.patch<ApiResponse<unknown>>(`/smb/${id}/profile/${section}`, { data }).then((r: any) => r.data.data),

  /** Intelligence Hub: trigger re-enrichment from Google Places, website scan, SerpAPI */
  requestEnrichment: (id: string) =>
    apiClient.post<ApiResponse<{ status: string; last_enriched_at?: string }>>(`/smb/${id}/enrich`).then((r: any) => r.data.data),
};

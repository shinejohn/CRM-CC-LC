      ...
      <div className={`flex-1 flex ${showThumbnails ? 'flex-row' : 'flex-col'} overflow-hidden`}>
        ...
        <div className={`${showThumbnails ? 'w-3/4' : 'w-full'} flex flex-col`}>
          ...
          <div className="flex-1 bg-gray-900 flex items-center justify-center p-2 relative">
            <img src={slides[currentSlide].content} alt={`Slide ${currentSlide + 1}`} className="w-full h-full object-contain" />
            ...
            <div className="absolute bottom-3 left-1/2 transform -translate-x-1/2 flex space-x-2 bg-black bg-opacity-60 rounded-full px-3 py-1.5">
              {slides.map((_, index) => <button key={index} className={`w-2 h-2 rounded-full transition-all ${index === currentSlide ? 'bg-white w-6' : 'bg-gray-400 hover:bg-gray-300'}`} onClick={() => goToSlide(index)}></button>)}
            </div>
          ...
          <div className="px-4 py-2 bg-yellow-900 bg-opacity-20 border-t border-yellow-800">
            <div className="text-xs font-medium text-yellow-200 mb-0.5">
              NOTES
            </div>
        ...
        {showThumbnails && <div className="w-1/4 border-l border-gray-700 overflow-y-auto bg-gray-800">

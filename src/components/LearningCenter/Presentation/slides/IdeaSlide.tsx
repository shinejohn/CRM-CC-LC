...
export const IdeaSlide: React.FC<IdeaSlideProps> = ({
  content,
  isActive,
  theme = 'blue',
}) => {
  ...
  return (
    ...
    >
      <div className="max-w-4xl mx-auto w-full">
        <Lightbulb size={64} className="mx-auto mb-6 text-yellow-500 animate-fade-in" />
        ...
        {content.ideas && (
          <ul className="space-y-4">
            {content.ideas.map((idea, index) => (
              <li key={index} className="bg-white rounded-lg p-6 shadow-md animate-fade-in" style={{ animationDelay: `${0.2 + index * 0.1}s` }}>
                <span className="text-gray-800 text-lg">{idea}</span>
              </li>
            ))}
          </ul>
        )}

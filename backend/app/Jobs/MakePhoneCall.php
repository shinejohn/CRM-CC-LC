...
final class MakePhoneCall implements ShouldQueue
{
    ...
    public function handle(PhoneService $phoneService): void
    {
        try {
            ...
            // A/B variant overrides the campaign script when assigned.
            $variant = $recipient->variant_id ? $recipient->variant : null;

            // Prepare call script
            $script = $variant?->message ?? $campaign->message;

            $templateId = $variant?->template_id ?? $campaign->template_id;

            // If template is used, render it
            if ($templateId) {
                $template = \App\Models\PhoneScript::find($templateId);
                if ($template) {
